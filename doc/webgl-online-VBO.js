...
function main() {
  ...
  vColor = color;\n\
    ...
    GL.bufferData(GL.ARRAY_BUFFER, size_vertices, GL.STATIC_DRAW);
    GL.bufferSubData(GL.ARRAY_BUFFER, 0, new Float32Array(vertices));
    GL.enableVertexAttribArray(0);
    GL.vertexAttribPointer(0, 2, GL.FLOAT, false, 4*(2+3), 0);
    GL.enableVertexAttribArray(1);
    GL.vertexAttribPointer(1, 3, GL.FLOAT, false, 4*(2+3), 2*4);
    
    /** 设置实例化数据 */
    VBO = GL.createBuffer ();
    GL.bindBuffer(GL.ARRAY_BUFFER, VBO);
    GL.bufferData(GL.ARRAY_BUFFER, size_offsets, GL.STATIC_DRAW);
    GL.bufferSubData(GL.ARRAY_BUFFER, 0, new Float32Array(offsets));
